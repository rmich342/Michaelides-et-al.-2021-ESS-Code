...
close all
%% post processing
along_track = csvread('repeat_dh_274_gt1l_2m_dem.csv', 1, 4); %along-track repeats
lon=along_track(1:end,1);
lat=along_track(1:end,2);
rgt = along_track(1:end, 22);
beam = along_track(1:end, 23); %beam 

plot_title = 'Track 274 gt2l:  4/18 to 10/16'; %title for the plots
track_name = '274_gt1l'; %for nameing files
...
hold on
f = scatter(lat_spaced, is2_corrected, 5, 'k', 'filled');
